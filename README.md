
# 👋 Hi, I'm Lakshman Krishnan  
### Spatial Data Specialist | GIS Consultant | ArcGIS Pro Expert  

📍 Based in Germany  
📧 lakshmankrishnan.gis@outlook.com  
🌐 Portfolio Website – Coming Soon  

---

## 🗺️ About Me

I specialize in transforming complex spatial data into clear, actionable insights.  
With hands-on expertise in ArcGIS Pro and spatial workflows, I support infrastructure, environmental, and planning projects with precise geospatial solutions.

---

## 🚀 GIS Services

- 🗺️ Professional Map Creation & Layout Design  
- 🔄 CAD (DWG) to GIS Data Conversion  
- 📍 Coordinate Reference System (CRS) Transformations  
- 📊 Spatial Analysis & Geoprocessing  
- 🧭 Georeferencing & Feature Digitization  
- 🛠️ Custom ArcGIS Pro Toolboxes  
- 📁 Data Cleaning, Structuring & Validation  

---

## 🛠️ Technical Stack

**Software**
- ArcGIS Pro  
- QGIS  
- AutoCAD (CAD Data Integration)  
- Microsoft Excel  

**Core Competencies**
- Raster & Vector Analysis  
- Spatial Joins & Overlay Analysis  
- Projection & Datum Management  
- Cartographic Design Principles  
- Infrastructure & Environmental Mapping  

---

## 📂 Featured Projects

- 🔹 DWG to Shapefile Conversion Workflow  
- 🔹 Climate Risk Assessment for Infrastructure  
- 🔹 Professional Cartographic Layout Portfolio  
- 🔹 Historical Map Georeferencing Project  

Explore my repositories below 👇  

---

## 📝 GIS Articles & Tutorials

- 📖 [Shapefile vs File Geodatabase – When to Use What?](https://github.com/spatial-studio/GIS-Articles/blob/main/shapefile-vs-file-geodatabase.md)
- 📖 Converting DWG to Shapefile – Best Practices  
- 📖 Understanding Coordinate Transformations  

---

## 📈 Current Focus

- GIS Automation & Workflow Optimization  
- Infrastructure Risk Mapping  
- Building a Data-Driven GIS Consultancy  

---

⭐ Available for freelance GIS projects and collaborations worldwide.

![Dragon Jump](https://raw.githubusercontent.com/spatial-studio/spatial-studio/main/assets/dragon-jump/dragon-jump.gif)

![Snake animation](https://raw.githubusercontent.com/spatial-studio/spatial-studio/output/github-contribution-grid-snake.svg)




